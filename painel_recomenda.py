...
#     col_class_orig = next((c for c in ["CLASSIFICAÇÃO", "CLASSIFICACAO", "Classificação"] if c in cols_obs), None)
#     col_oferta_orig = next((c for c in ["Oferta Senai", "OFERTA_SENAI", "OFERTA SENAI"] if c in cols_obs), None)
...
# ──────────────────────────────────────────────────────────────────────────
warnings.filterwarnings("ignore")
st.set_page_config(page_title="Recomendação de Curso | SENAI Bahia", layout="wide")
...
def apply_custom_styles():
    ...
    .kpi-box {
        ...
        flex: 1; min-width: 150px; border-left: 5px solid #DDD;
    }
    .kpi-title { color: #666; font-size: 0.70rem; text-transform: uppercase; font-weight: 700; margin-bottom: 8px; }
    .kpi-value { color: #1A1A1A; font-size: 1.6rem; font-weight: 800; line-height: 1; }
    .kpi-delta { font-size: 0.75rem; margin-top: 5px; }

    /* Títulos */
    .page-title { font-family: 'Barlow Condensed', sans-serif; font-size: 2.2rem; font-weight: 700; color: #1a1a1a; }
    .page-sub { font-size: 0.9rem; color: #666; margin-bottom: 2rem; }
    .section-title { font-family: 'Barlow Condensed', sans-serif; font-size: 1.2rem; font-weight: 600; text-transform: uppercase; color: #004587; margin-top: 20px; }
    </style>
    """, unsafe_allow_html=True)
...
# ──────────────────────────────────────────────────────────────────────────
# 2. MOTOR DE DADOS (CARREGAMENTO E PREPARAÇÃO)
# ──────────────────────────────────────────────────────────────────────────
@st.cache_data
def load_and_process_data():
    # Carregamento dos Pickles
    with open("BD_Plan_Curso_Tecnico.pkl","rb") as f: bd = pickle.load(f)
    with open("Base_Concorrentes.pkl","rb") as f: conc = pickle.load(f)
    with open("Base_de_Evasao.pkl","rb") as f: ev = pickle.load(f)
    with open("dVagas.pkl","rb") as f: vagas = pickle.load(f)
    ...
    # --- Join 1: Evasão ---
    ev_agg = ev.groupby("COD_Base_de_Evasao").agg(
        EVASAO_PAG=("EVASAO", lambda x: x[ev.loc[x.index,"CONDIÇÃO"]=="PAGANTE"].sum()),
        EVASAO_BOLS=("EVASAO", lambda x: x[ev.loc[x.index,"CONDIÇÃO"]=="GRATUITO"].sum())
    ).reset_index()
    df = bd.merge(ev_agg, on="COD_Base_de_Evasao", how="left")

    # --- Join 2: Vagas (Defensivo) ---
    if not vagas.empty:
        vagas_m = vagas.rename(columns={"VAGAS_2": "VAGAS_ULTIM"}).drop_duplicates(subset=["COD_VAGAS"])
        df = df.merge(vagas_m[["COD_VAGAS", "VAGAS_ULTIM"]], on="COD_VAGAS", how="left")
        df["VAGAS_ULTIM"] = pd.to_numeric(df["VAGAS_ULTIM"], errors='coerce').fillna(0)

    # --- Join 3: Observatório (Normalização de Nomes) ---
    if not obs.empty:
        obs_cols = obs.columns
        col_class = next((c for c in ["CLASSIFICACAO", "CLASSIFICAÇÃO"] if c in obs_cols), None)
        col_oferta = next((c for c in ["OFERTA_SENAI", "Oferta Senai"] if c in obs_cols), None)
        
        subset_obs = ["COD_Observatorio"]
        rename_map = {}
        if col_class: 
            subset_obs.append(col_class)
            rename_map[col_class] = "CLASSIFICACAO"
        if col_oferta: 
            subset_obs.append(col_oferta)
            rename_map[col_oferta] = "OFERTA_SENAI"
            
        obs_clean = obs[subset_obs].drop_duplicates(subset=["COD_Observatorio"]).rename(columns=rename_map)
        df = df.merge(obs_clean, on="COD_Observatorio", how="left")

    # --- Join 4: Concorrentes (Anti-Duplicação) ---
    conc_agg = conc.groupby("COD_Concorrentes")["INSTITUIÇÃO"].nunique().reset_index(name="QTD_CONCORRENTES")
    df = df.merge(conc_agg, on="COD_Concorrentes", how="left")
...
# ──────────────────────────────────────────────────────────────────────────
# 3. LÓGICA DE CÁLCULO (MÉTRICAS TEMPORAIS)
# ──────────────────────────────────────────────────────────────────────────
def compute_temporal_metrics(g):
    anos_ref = [2023, 2024, 2025, 2026]
    
    def get_trend_string(cond, col, is_sum=True):
        vals = []
        for ano in anos_ref:
            m = (g["ANO"] == ano)
            if cond: m &= (g["CONDIÇÃO"] == cond)
            res = g.loc[m, col]
            val = (res.sum() if is_sum else res.max()) if not res.empty else 0
            vals.append(int(round(float(pd.NA if pd.isna(val) else val))))
        
        if sum(vals) == 0: return "-"
        
        res_str = []
        for i, v in enumerate(vals):
            if v == 0: res_str.append("-")
            elif i > 0 and vals[i-1] > 0:
                icon = "🟢" if v > vals[i-1] else "🔴" if v < vals[i-1] else ""
                res_str.append(f"{icon}{v}")
            else: res_str.append(str(v))
        return " | ".join(res_str)

    return pd.Series({
        "MAT. PAGANTE": get_trend_string("PAGANTE", "Valor"),
        "MAT. BOLSISTA": get_trend_string("GRATUITO", "Valor"),
        "EV. PAGANTE": get_trend_string("PAGANTE", "EVASAO_PAG"),
        "VAGAS": get_trend_string(None, "VAGAS_ULTIM", False),
        "CONCORRENTES": int(g["QTD_CONCORRENTES"].max() if "QTD_CONCORRENTES" in g.columns else 0)
    })

# ──────────────────────────────────────────────────────────────────────────
# 4. INTERFACE: SIDEBAR E FILTROS
# ──────────────────────────────────────────────────────────────────────────
...
with st.sidebar:
    st.image("https://www.fieb.org.br/senai/wp-content/themes/senai/img/logo-senai.png", width=180)
    st.markdown("### ✦ CENTRO DE COMANDO")
    
    with st.expander("📅 TEMPORAL", expanded=True):
        ano_sel = multiselect_all("ANO", sorted(df_raw["ANO"].unique()), "ano")
        sem_sel = multiselect_all("SEMESTRE", sorted(df_raw["SEMESTRE"].unique()), "sem")
    
    with st.expander("🌎 GEOGRÁFICO", expanded=True):
        reg_sel = multiselect_all("REGIONAL", sorted(df_raw["REGIONAL"].unique()), "reg")
        uni_sel = multiselect_all("UNIDADE", sorted(df_raw.loc[df_raw["REGIONAL"].isin(reg_sel), "UNIDADE"].unique()), "uni")

    with st.expander("📚 EDUCACIONAL"):
        mod_sel = multiselect_all("MODALIDADE", sorted(df_raw["MODALIDADE"].unique()), "mod")
        tur_sel = multiselect_all("TURNO", sorted(df_raw["TURNO"].unique()), "tur")

# --- Aplicação das Máscaras ---
mask_base = (df_raw["SEMESTRE"].isin(sem_sel) & df_raw["UNIDADE"].isin(uni_sel) & 
             df_raw["MODALIDADE"].isin(mod_sel) & df_raw["TURNO"].isin(tur_sel))

df_kpi = df_raw[mask_base & df_raw["ANO"].isin(ano_sel)]
df_tab = df_raw[mask_base] # Tabela ignora filtro de ano para mostrar o histórico completo

# ──────────────────────────────────────────────────────────────────────────
# 5. CONTEÚDO PRINCIPAL (KPIs E TABELA)
# ──────────────────────────────────────────────────────────────────────────
st.markdown('<div class="page-title">📊 RECOMENDAÇÃO DE CURSO</div>', unsafe_allow_html=True)
st.markdown('<div class="page-sub">SENAI Bahia · Inteligência de Mercado</div>', unsafe_allow_html=True)

# --- KPIs com Deltas ---
def render_kpi(col, label, val, delta_val, color):
    delta_html = f"<div class='kpi-delta'>{delta_val}</div>" if delta_val else ""
    col.markdown(f"""
        <div class="kpi-box" style="border-left-color: {color};">
            <div class="kpi-title">{label}</div>
            <div class="kpi-value">{val}</div>
...
# Lógica Simplificada de KPI
cur_mp = df_kpi[df_kpi["CONDIÇÃO"]=="PAGANTE"]["Valor"].sum()
cur_mb = df_kpi[df_kpi["CONDIÇÃO"]=="GRATUITO"]["Valor"].sum()
cur_conc = df_kpi["QTD_CONCORRENTES"].max() if "QTD_CONCORRENTES" in df_kpi.columns else 0

cols = st.columns(5)
render_kpi(cols[0], "Mat. Pagante", int(cur_mp), None, "#004587")
render_kpi(cols[1], "Mat. Bolsista", int(cur_mb), None, "#004587")
render_kpi(cols[2], "Concorrentes", int(cur_conc), None, "#00A199")

# --- Filtros de Topo (Observatório) ---
st.divider()
c_obs1, c_obs2 = st.columns([1, 1])
with c_obs1:
    obs_opts = sorted(df_raw["CLASSIFICACAO"].dropna().unique()) if "CLASSIFICACAO" in df_raw.columns else []
    sel_obs = st.multiselect("🏷️ Classificação Preditiva", obs_opts, default=obs_opts)
with c_obs2:
    sel_of = st.pills("🚀 Oferta SENAI", options=["Sim", "Não"], selection_mode="single")

# Filtragem Final da Tabela
if "CLASSIFICACAO" in df_tab.columns: df_tab = df_tab[df_tab["CLASSIFICACAO"].isin(sel_obs)]
if sel_of and "OFERTA_SENAI" in df_tab.columns: df_tab = df_tab[df_tab["OFERTA_SENAI"] == sel_of]

# --- Renderização da Tabela ---
st.markdown('<div class="section-title">Detalhamento Temporal por Curso</div>', unsafe_allow_html=True)
st.caption("Frequência: 2023 | 2024 | 2025 | 2026")

group_cols = ["CURSO", "MODALIDADE", "TURNO"]
for c in ["CLASSIFICACAO", "OFERTA_SENAI"]:
    if c in df_tab.columns: group_cols.append(c)

df_final = df_tab.groupby(group_cols, dropna=False).apply(compute_temporal_metrics).reset_index()

interactive_table(df_final, paging=False, scrollY="500px", scrollX=True)

# ──────────────────────────────────────────────────────────────────────────
# 6. EXPORTAÇÃO
# ──────────────────────────────────────────────────────────────────────────
st.divider()
if st.button("⬇️ Gerar Relatório Excel"):
    buf = BytesIO()
    df_final.to_excel(buf, index=False)
    st.download_button("Baixar Arquivo", buf.getvalue(), "recomendacao_senai.xlsx")